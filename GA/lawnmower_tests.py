    ...
    def __init__(self, genes):
        ...
        for func in funcs:
            for index in reversed(range(len(func.Ops))):
                if type(func.Ops[index]) is Call and \
                        func.Ops[index].FuncId is not None:
                    if func.Ops[index].FuncId >= len(funcs) or \
                        len(funcs[func.Ops[index].FuncId].Ops) == 0:
                            del func.Ops[index]
                            
        for index in reversed(range(len(temp))):
            if type(temp[index]) is Call and temp[index].FuncId is not None:
                if temp[index].FuncId >= len(funcs) or \
                    len(funcs[temp[index].FuncId].Ops) == 0:
                        del temp[index]
