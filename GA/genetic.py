...
def tournament(generate_parent, crossover, compete, display, sort_key,
    ...
    pool = [[generate_parent(), [0, 0, 0]]
        for _ in range(1 + numParents * numParents)]
...
def _crossover(parentGenes, index, parents, get_fitness,
               crossover, mutate, generate_parent):
...
def get_best(get_fitness, targetLen, optimalFitness, geneSet, display,
    ...
    strategyLookup = {
            ...
            Strategies.Crossover: lambda p, i, o: 
                _crossover(p.Genes, i, o, get_fitness, crossover,
                           fnMutate, fnGenerateParent)
            }
    ...
    for timedOut, improvement in \
            _get_improvement(fnNewChild, fnGenerateParent, maxAge,
                             poolSize, maxSeconds):
        if timedOut:
            return improvement
        display(improvement)
