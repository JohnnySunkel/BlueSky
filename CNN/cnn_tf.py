...
def main():
    ...
    X = tf.placeholder(tf.float32, 
                       shape = (batch_sz, 32, 32, 3), name = 'X')
    T = tf.placeholder(tf.float32,
                       shape = (batch_sz, K), name = 'T')
    ...
    train_op = tf.train.RMSPropOptimizer(0.0001, 
                                         decay = 0.99,
        ...
        for i in range(max_iter):
            for j in range(n_batches):
                ...
                if len(Xbatch) == batch_sz:
                    session.run(train_op, feed_dict = {X: Xbatch,
                                                       T: Ybatch})
                    if j % print_period == 0:
                        ...
                        for k in range(len(Xtest) // batch_sz):
                            Xtestbatch = Xtest[k * batch_sz:(
                                    k * batch_sz + batch_sz),]
                            Ytestbatch = Ytest_ind[k * batch_sz:(
                                    k * batch_sz + batch_sz),]
