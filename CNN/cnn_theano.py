...
def convpool(X, W, b, poolsize = (2, 2)):
    ...
    # first reshape it to a tensor of shape (1, n_filters, 1, 1).
    # Each bias will thus be broadcasted across mini-batches and
...
def main():
    ...
    # W1_shape = (num_feature_maps, num_color_channels,
    ...
    # W2_shape = (num_feature_maps, old_num_feature_maps, 
    #             filter_width, filter_height)
    W2_shape = (50, 20, 5, 5)
    ...
    # Vanilla ANN weights
    W3_init = np.random.randn(W2_shape[0] * 5 * 5, M) / np.sqrt(
            W2_shape[0] * 5 * 5 + M)
    ...
    dparams = [
            theano.shared(
                    np.zeros_like(
                            p.get_value(),
                            dtype = np.float32)) for p in params]
    ...
    # Create another function for this because we want
    # it over the whole dataset
